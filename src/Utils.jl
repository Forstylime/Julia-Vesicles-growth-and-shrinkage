...
"表面能密度 f_surf"
function f_surf(phi, phi_hat, ops::Operators, conf::Config)
...
"弯曲能密度 f_bend"
function f_bend(phi, phi_hat, ops::Operators, conf::Config)
    ε = conf.epsilon
...
function calculate_area(phi, phi_hat, ops::Operators, conf::Config)
    return integrate(f_surf(phi, phi_hat, ops, conf), conf)
end
...
function compute_original_energy(state::FieldState, ops::Operators, conf::Config)
    ...
    F_s = conf.gamma_surf * integrate(f_surf(state.phi, state.phi_hat, ops, conf), conf)
    F_b = conf.gamma_bend * integrate(f_bend(state.phi, state.phi_hat, ops, conf), conf)
    F_o = integrate(f_osmotic(state.phi, state.psi, conf), conf)

    A_current = calculate_area(state.phi, state.phi_hat, ops, conf)
    F_a = 0.5 * conf.gamma_area * (A_current - state.A)^2

    return kinetic + conf.lambda * (F_s + F_b + F_o + F_a)  # 修正：lambda
end
...
"ω = (φ³-φ)/ε - ε·Δφ"
function get_omega(phi, phi_hat, ops::Operators, conf::Config)
    ε = conf.epsilon
    lap_phi = real(ops.ifft_plan * (ops.Laplacian .* phi_hat))
...
function get_W1(phi, phi_hat, ops::Operators, conf::Config)
    A_cur = calculate_area(phi, phi_hat, ops, conf)
    W1    = 0.5 * conf.gamma_area * (A_cur - conf.A)^2
    W1 + conf.C1 < 0 && error("FATAL: W1 + C1 < 0，C1 偏小！W1 = $W1")
    return W1
end

function get_W2(phi, phi_hat, ops::Operators, conf::Config)
    F_surf_val = conf.gamma_surf * integrate(f_surf(phi, phi_hat, ops, conf), conf)
    F_bend_val = conf.gamma_bend * integrate(f_bend(phi, phi_hat, ops, conf), conf)
...
function get_H1(phi, phi_hat, ops::Operators, conf::Config)
    W1      = get_W1(phi, phi_hat, ops, conf)
    R1      = sqrt(W1 + conf.C1)
    A_cur   = calculate_area(phi, phi_hat, ops, conf)
    omega   = get_omega(phi, phi_hat, ops, conf)
    factor  = conf.gamma_area * (A_cur - conf.A) * (3 * sqrt(2) / 4)
    return @. (factor * omega) / R1
end

function get_H2(phi, phi_hat, ops::Operators, conf::Config)
    W2    = get_W2(phi, phi_hat, ops, conf)
    R2    = sqrt(W2 + conf.C2)
    ε     = conf.epsilon
    omega = get_omega(phi, phi_hat, ops, conf)
    ...
    H2_SB = @. conf.gamma_surf * (3 * sqrt(2) / 4) * omega +
